...
class ReleaseVersionTests(unittest.TestCase):
    ...
    def test_python_package_is_the_2_0_0_release(self) -> None:
    ...
    def test_windows_resources_and_installer_use_the_same_version(self) -> None:
        ...
        self.assertEqual(version_info.count("u'2.0.0'"), 2)
        self.assertIn('!define APP_VERSION "2.0.0"', installer)

    def test_release_manifest_and_notes_are_versioned_for_2_0_0(self) -> None:
        manifest = self._text("scripts/create-release-manifest.ps1")
        self.assertIn("Pix2TexStudio-2.0.0-Setup.exe", manifest)
        self.assertIn("version = '2.0.0'", manifest)
        self.assertTrue(
            (self.project_root / "docs/release/RELEASE_NOTES_2.0.0.md").is_file()
        )
